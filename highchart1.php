...
if(isset($_POST['search'])){
				...
				$sql_sum="SELECT price,nameproduct,count(tb_basket_detail.id_product) as sum_product FROM tb_basket_detail
				inner join tb_basket on tb_basket.id_basket=tb_basket_detail.id_basket
				inner join tb_product on tb_product.id_product=tb_basket_detail.id_product
				where basket_date BETWEEN '$start_date' AND '$end_date'
					group by nameproduct";
...
?>
				<tr>
					<td><?php echo $obj_sum['nameproduct']; ?></td>
					<td><?php echo ($obj_sum['sum_product'] * $obj_sum['price']); ?></td>				
				</tr>
				<?php } ?>		
			</tbody>
		</table>
...
format: '{point.y}'
		...
		title: {
			text: 'แผนภูมิแท่งแสดงจำนวนการขายสินค้า'
		},
		yAxis: {
			allowDecimals: false,
			title: {
